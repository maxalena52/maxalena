...
export function applyBookCopy<T extends Pick<Book, "slug"> & Partial<Book>>(book: T): T {
  const extra = COPY[book.slug];
  return extra ? { ...book, ...extra } : book;
}
